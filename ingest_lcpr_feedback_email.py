...
def communications_of_the_day():
    ...
    email_sends = spark.sql(f"""
                  ...
                  left join ( select concat('LCPR_FX_',t1.sub_acct_no) sub_acct_no, t1.offer_type, t1.use_case, t1.day, t1.id_oferta_int
                             from db_dev_cdp_project.lcpr_offers_in t1
                             inner join (
                                         select sub_acct_no,  offer_type, channel, use_case, max(day) max_day
                                         from db_dev_cdp_project.lcpr_offers_in
                                         where offer_type='single' and day <= '{hoy}'
                                         group BY sub_acct_no, offer_type, channel, use_case
                                         ) 
                                     t2 on t1.sub_acct_no=t2.sub_acct_no and t1.use_case=t2.use_case and t1.day=t2.max_day and t1.channel = 'email' and t1.offer_type='single' and t1.channel=t2.channel
...
def update_from_clicks():
    ...
    email_clicks = spark.sql(f""" SELECT o.contact_id contact_id_click
                                 ...
                                 INNER JOIN  db_dev_cdp_project.communications_hist c
                                 ON (c.contact_id = o.contact_id) and c.campaign_id  = cast(o.campaign_id as integer) and cast(o.email_sent_at as date) =  to_date('{hoy}', 'yyyyMMd') 
                                 WHERE datediff(cast(o.event_time as date),cast(o.email_sent_at as date)) <= 14
...
def update_from_bounces():
    ...
    email_bounces = spark.sql(f""" select b1.contact_id contact_id_bounce, b1.campaign_id campaign_id_bounce, b1.email_sent_at, b1.event_time, b1.bounce_type type, b1.dsn_reason reason
                                   ...
                                   inner join   db_dev_cdp_project.communications_hist c
                                      on (c.contact_id = b1.contact_id) and c.campaign_id  = cast(b1.campaign_id as integer) and cast(b1.email_sent_at as date) =  to_date('{hoy}', 'yyyyMMd') 
