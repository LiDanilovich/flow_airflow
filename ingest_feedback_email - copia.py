...
last14 = (datetime.today()-timedelta(days=days_before)-timedelta(days=13)).strftime('%Y-%m-%d')
...
def update_from_opens():
    ...
    comm = spark.sql(f""" SELECT * from db_dev_cdp_project.communications_hist where comm_dt>=cast('{last14}' as date) and channel='email' """)
