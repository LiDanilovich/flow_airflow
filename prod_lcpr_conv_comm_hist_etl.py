...
def MyTransform(glueContext, dfc) -> DynamicFrameCollection:
    ...
    df = df.withColumn("conv_dt_ms", F.unix_timestamp(   F.to_timestamp("conv_dt", "yyyy-MM-dd HH:mm:ss")  ) * 1000).drop("conv_dt")
...
if ('--{}'.format('days_before') in sys.argv):
    ...
    minp = {'max_partition': (datetime.today()-timedelta(days=days_before)-timedelta(days=13)).strftime('%Y%m%d')}
else:
    maxp = {'max_partition': (datetime.today()-timedelta(days=0)).strftime('%Y%m%d')}
    minp = {'max_partition': (datetime.today()-timedelta(days=13)).strftime('%Y%m%d')}


max_partition = maxp['max_partition']
min_partition = minp['max_partition']

print('Insertando datos desde {0} al {1} '.format(min_partition, max_partition))

q_max_partition = f"""select * from df where day >= '{min_partition}' and day <= '{max_partition}';"""
...
ApplyMapping_node_toa = ApplyMapping.apply(
    ...
    mappings=[
        ...
        ("conversion_dt", "string", "conv_dt", "string"),
        ...
        ("conv_flg", "boolean", "conv_flg", "boolean"),
...
pre_query = "begin;"
pre_query += f"delete from prod.public.lcpr_communications_hst where dt >= {min_partition} and dt <= {max_partition};"
pre_query += "end;"
